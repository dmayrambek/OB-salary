// ==========================================================================
// 🧠 БАЗА ДАННЫХ ДЛЯ ИНТЕРАКТИВНОЙ СХЕМЫ (ТЕКУЩАЯ СХЕМА ЗПП)
// ==========================================================================
const nodeData = {
    node1: {
        ru: {
...
function setLang(lang) {
  ...
  // 🔥 ОБНОВЛЕНИЕ СХЕМЫ: Если мы на странице схемы, перерисовываем правую панель
  renderSchemeDetails(activeNodeKey);
}
...
// ==========================================================================
// 🕹️ ЛОГИКА ИНТЕРАКТИВНОЙ СХЕМЫ
// ==========================================================================

// Функция рендера текста на боковой стеклянной панели
function renderSchemeDetails(key) {
  const container = document.getElementById('details-content');
  if (!container) return; // Защита, если мы находимся на другой странице проекта
  
  const data = nodeData[key][currentLang];
  ...
  const painHeader = currentLang === 'ru' ? 'Проблемы и узкие места' : 'Bottlenecks & Pain Points';
...
// Функция, которая срабатывает при клике на любой узел в scheme.html
function showDetails(nodeId, element) {
  activeNodeKey = nodeId;
  // Снимаем класс active со всех карточек на схеме
  document.querySelectorAll('.node-card').forEach(card => card.classList.remove('active'));
  // Подсвечиваем текущую карточку
  element.classList.add('active');
  // Перерисовываем боковую панель
  renderSchemeDetails(nodeId);
}
...
function filterTable(type) {
  ...
  if (btnAll && btnAdv && btnGrowth) {
    ...
    if (type === 'all') btnAll.add('active');
    if (type === 'advantage') btnAdv.add('active');
    if (type === 'growth') btnGrowth.add('active');
  }
...
// ==========================================================================
// 🚀 ЗАПУСК ПОСЛЕ ПОЛНОЙ ЗАГРУЗКИ DOM СТРАНИЦЫ
// ==========================================================================
document.addEventListener('DOMContentLoaded', () => {
  // 1. Запускаем базовый перевод страниц
  setLang(currentLang);
  
  // 2. Принудительно инициализируем первый узел, если мы на странице со схемой
  if (document.getElementById('details-content')) {
    renderSchemeDetails('node1');
  }
});
